"""
This is the evaluation script for the results of raw/debug ToGs and LLM QA.
This code is modified from the original evaluation script of ToG, which contains some bugs.
"""
...
def main(args):
    ...
    for answer in answer_json:
        ...
        ans_list = re.findall(r'\{(.*?)\}', results)
        ans_list = re.findall(r'\{(.*)', results) if len(ans_list) == 0 else ans_list
        if answer_match(reference=reference_answer, answers=ans_list):
            num_right += 1
        else:
            num_error += 1
