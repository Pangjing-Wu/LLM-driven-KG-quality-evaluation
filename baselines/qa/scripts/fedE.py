...
from baselines.qa.utils import extract_entities_relations
...
def parse_args():
    ...
    parser.add_argument('--fed_update_epoch', type=int, default=100, help='federated update epochs')
...
def fed_aggregation(model_0, model_1, shared_entity_index, shared_relation_index):
    with torch.no_grad():
        entity_emb_0 = model_0.entity_embeddings.weight[shared_entity_index]
        entity_emb_1 = model_1.entity_embeddings.weight[shared_entity_index]
        relation_emb_0 = model_0.relation_embeddings.weight[shared_relation_index]
        relation_emb_1 = model_1.relation_embeddings.weight[shared_relation_index]
        ave_entity     = (entity_emb_0 + entity_emb_1) / 2
        ave_relation   = (relation_emb_0 + relation_emb_1) / 2
        model_0.entity_embeddings.weight[shared_entity_index] = ave_entity
        model_1.entity_embeddings.weight[shared_entity_index] = ave_entity
        model_0.relation_embeddings.weight[shared_relation_index] = ave_relation
        model_1.relation_embeddings.weight[shared_relation_index] = ave_relation
...
def main(args, result_path, logger):
    ...
    logger.info(f'start training KGE model for {args.epoch} epochs.')
    for e in range(args.epoch):
        ...
        if e % args.fed_update_epoch == 0:
            logger.info(f'federate update at epoch {e}.')
            fed_aggregation(model_0.model, model_1.model, shared_entity_index, shared_relation_index)
...
if __name__ == "__main__":
    ...
    result_dir  = f'./results/KG-quality-eval/baselines/FedE-{args.kge}/{args.kg0}-{args.kg1}'
    result_path = os.path.join(result_dir, f'{date}-results.json')
    os.makedirs(os.path.dirname(result_dir), exist_ok=True)
    
    level  = 'DEBUG' if args.debug else 'INFO'
    logger = set_logger(f'Fede-kg-eval-{date}', logfile=result_path.replace('.json', '.log'), level=level)
